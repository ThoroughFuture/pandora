...
class WholeSlideImage(object):
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up=255, mthresh=7, close=0, use_otsu=False,
        ...
        nn = len(self.level_dim)
        self.level_dim = list(self.level_dim)
        self.level_downsamples = list(self.level_downsamples)
        w_o, h_o = self.level_dim[0]
        print('seg_level', seg_level)
        print('before')
        print(self.level_dim)  # ((37440, 63732),)
        print(self.level_downsamples)  # ((1.0, 1.0),(2.0, 2.0),(4.0, 4.0),(8.0, 8.0))
        for i in range(seg_level + 1):
            if i < len(self.level_dim):
                continue
            self.level_downsamples.append((2 ** i, 2 ** i))
            self.level_dim.append((w_o // (2 ** i), h_o // (2 ** i)))
        print("after")
        print(self.level_dim)  # ((37440, 63732),)
        print(self.level_downsamples, flush=True)  # ((1.0, 1.0),(2.0, 2.0),(4.0, 4.0),(8.0, 8.0))
        print(self.level_dim[seg_level])
        # img = np.array(self.wsi.read_region((0, 0), seg_level, self.level_dim[seg_level]))
        if seg_level > nn:
            tmp_level = nn - 1
            print('tmp_level', tmp_level)
            img = np.array(self.wsi.read_region((0, 0), tmp_level, self.level_dim[tmp_level]).convert("RGB"))
            print('img_org', img.shape)
            img = cv2.resize(img, self.level_dim[seg_level])
        else:
            img = np.array(self.wsi.read_region((0, 0), seg_level, self.level_dim[seg_level]).convert("RGB"))
        print('img_fin', img.shape)

        self.img_seg = copy.deepcopy(img)
        ...
        print("mix threshold", flush=True)
        img = cv2.medianBlur(img, 9)  # 均值滤波
        img_hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)  # Convert to HSV space
        # lower_color = np.array([125, 0, 0])
        lower_color = np.array([125, 10, 10])
        upper_color = np.array([185, 255, 255])
        img_norm = cv2.inRange(img_hsv, lower_color, upper_color)
        # img_med = cv2.medianBlur(img_hsv[:, :, 1], mthresh)  # Apply median blurring
        _, img_tr = cv2.threshold(img_hsv[:, :, 1], 0, sthresh_up, cv2.THRESH_TRIANGLE + cv2.THRESH_BINARY)
        img_otsu = img_norm & img_tr
    ...
    def visWSI(self, vis_level=0, color=(0, 255, 0), hole_color=(0, 0, 255), annot_color=(255, 0, 0),
        ...
        if vis_level == self.seg_level:
            print("img=self.img_seg")
            img = copy.deepcopy(self.img_seg)
        else:
            print("vis_level")
            nn = len(self.wsi.level_dimensions)
            self.level_dim = list(self.level_dim)
            self.level_downsamples = list(self.level_downsamples)
            w_o, h_o = self.level_dim[0]
            ...
            print(self.level_dim)  # ((37440, 63732),)
            print(self.level_downsamples, flush=True)  # ((1.0, 1.0),(2.0, 2.0),(4.0, 4.0),(8.0, 8.0))
            print(self.level_dim[vis_level])
            # img = np.array(self.wsi.read_region((0, 0), seg_level, self.level_dim[seg_level]))
            if vis_level > nn:
                tmp_level = nn - 1
                print('tmp_level', tmp_level)
                img = np.array(self.wsi.read_region((0, 0), tmp_level, self.level_dim[tmp_level]).convert("RGB"))
                print('img_org', img.shape)
                img = cv2.resize(img, self.level_dim[vis_level])
            else:
