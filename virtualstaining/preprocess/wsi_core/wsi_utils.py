...
def DrawMapFromCoords(canvas, wsi_object, coords, patch_size, vis_level, indices=None, draw_grid=True):
    ...
    if indices is None:
        indices = np.arange(len(coords))
    total = len(indices)
    tmp_wsi = wsi_object.img_seg
    print('tmp_wsi', tmp_wsi.shape)
    ...
    for idx in tqdm(range(total)):
        ...
        y, x = coord
        patch = tmp_wsi[x:x + patch_size[1], y:y + patch_size[0], :]

        canvas_crop_shape = canvas[x:x + patch_size[1], y:y + patch_size[0], :3].shape[:2]
        canvas[x:x + patch_size[1], y:y + patch_size[0], :3] = patch[:canvas_crop_shape[0], :canvas_crop_shape[1], :]
        if draw_grid:
            DrawGrid(canvas, coord, patch_size)
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=16, draw_grid=False, bg_color=(0, 0, 0), alpha=-1):
    ...
    vis_level = wsi_object.seg_level
    print(wsi.level_dimensions[0])
    w, h = wsi_object.level_dim[vis_level]
    print('Stitch vis_level', vis_level)
    print('downscaled size for stiching: {} x {}'.format(w, h))
