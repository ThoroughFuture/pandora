...
class convnextv2_T(nn.Module):
    def __init__(self, convnextv2_pth="/path/to/backbone_pretrained.pt"):
        super().__init__()
        self.convnextv2 = ConvNeXtV2(depths=[3, 3, 9, 3], dims=[96, 192, 384, 768])
        self.convnextv2.head = nn.Sequential()
        self.convnextv2.load_state_dict(torch.load(convnextv2_pth, map_location=torch.device('cpu')))
